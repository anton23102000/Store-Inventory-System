...
{
    private readonly List<Beverage> Beverages;
    ...
    public void AddBeverage()
    {
        ...
        var beveragePrice = Convert.ToDouble(beveragePR);
    ...
    public void RemoveBeverage()
    {
        ...
        if (!found) Console.WriteLine("Beverage not found.");
    }

    public void ShowBeverages()
    {
        var sum = 0;
        foreach (var beverage in Beverages) sum += beverage.Quantity;
        Console.WriteLine($"Fish contain{sum}kilograms");
        foreach (var beverage in Beverages) Console.WriteLine($"{beverage.Name} - {beverage.Quantity}kg");
    }
}