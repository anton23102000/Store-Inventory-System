...
{
    private readonly List<Meat> SMeat;
    ...
    public void AddMeat()
    {
        ...
        var meatPrice = Convert.ToDouble(meatPR);
    ...
    public void RemoveMeat()
    {
        ...
        if (!found) Console.WriteLine("Meat not found.");
    }

    public void ShowMeat()
    {
        var sum = 0;
        foreach (var meat in SMeat) sum += meat.Quantity;
        Console.WriteLine($"Meat contain{sum}kilograms");
        foreach (var meat in SMeat) Console.WriteLine($"{meat.Name} - {meat.Quantity}kg");
    }
}