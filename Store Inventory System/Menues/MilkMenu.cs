...
{
    private readonly List<Milk> SMilk;
    ...
    public void AddMilk()
    {
        ...
        var milkPrice = Convert.ToDouble(milkPR);
    ...
    public void RemoveMilk()
    {
        ...
        if (!found) Console.WriteLine("Milk not found.");
    }

    public void ShowMilk()
    {
        var sum = 0;
        foreach (var milk in SMilk) sum += milk.Quantity;
        Console.WriteLine($"Milk contain{sum}bottles");
        foreach (var milk in SMilk) Console.WriteLine($"{milk.Name} - {milk.Quantity}");
    }
}